...
class Saxophone extends stream.Writable {
    ...
    write(input, isLast = false) {
        ...
        while (localPosition < end) {
            ...
            if (input[localPosition] === '/') {
                const tagName = input.slice(localPosition + 1, tagClose);
                const stackedTagName = this.tagStack.pop();
                if (stackedTagName !== tagName) {
                    this.emit('error', new Error(`Unclosed tag: ${stackedTagName}`));
        ...
        if (this.tagStack.length !== 0) {
            this.emit('error', new Error(`Unclosed tags: ${this.tagStack.join(',')}`));
        }
        this.emit('end');
        return this;
    }
